...
int yylex();
nodeType* addLeaf(char* label, char* ptr);
nodeType* addNode(char* label, int num_args, ...);
int yyerror(char*);
nodeType* root = NULL;
%}
...
statements: 
    ...
    ;
statement:
    assignment_stmt {$$=addNode("assignment-stmt", 1, $1);} 
    | function_call {$$=addNode("function-call", 1, $1);}
    | if_stmt       {$$=addNode("if-stmt", 1, $1);}
    | while_stmt    {$$=addNode("while-stmt", 1, $1);}
    | error {printf("[Yacc] Failure :-(\n"); yyerrok; yyclearin;}
    ;
assignment_stmt:
    lBracket equal identifier parameters rBracket       {if(DEBUG){printf("[Yacc] \nassignment_stmt\n\n\n");}; $$=addNode("assignment-stmt", 5, addLeaf(NULL, $1), addLeaf(NULL, $2), addLeaf("assignment-id", $3), $4, addLeaf(NULL, $5));}
    ;
function_call:
